...
import { PenTool, Clock, CheckCircle2, AlertCircle, Plus, Search } from 'lucide-react';

// Hardcoded Data simulate Dynamic Approval Levels
const mockAssetRequests = [
  { 
    id: 'AST-001', item: 'Drone DJI Agras T40', quantity: '2 Unit', date: '14 Jun 2026', requester: 'Dimas Pradipta', role: 'Kepala Operasional', status: 'pending', department: 'Manajemen Kebun', 
    currentStepIndex: 3, // Multi-level approval: At CEO Review
    trackingSteps: ['Diajukan', 'Review Manager', 'Review Direktur', 'Review CEO', 'Proses Beli', 'Pengiriman', 'Selesai']
  },
  { 
    id: 'AST-002', item: 'Cangkul Tarik', quantity: '50 Pcs', date: '08 Jun 2026', requester: 'Budi Santoso', role: 'Staff Operasional', status: 'approved', department: 'Pertanian', 
    currentStepIndex: 3, // Completed
    trackingSteps: ['Diajukan', 'Review SPV', 'Proses Beli', 'Selesai']
  },
  { 
    id: 'AST-003', item: 'Sensor IoT Soil Moisture', quantity: '15 Unit', date: '01 Jun 2026', requester: 'Reza Fahlevi', role: 'Teknisi IT', status: 'approved', department: 'IT Support', 
    currentStepIndex: 4, // Completed
    trackingSteps: ['Diajukan', 'Review Manager', 'Review GM', 'Proses Beli', 'Selesai']
  },
];

const AssetRequest = () => {
  return (
    <DashboardLayout pageTitle="Permintaan Alat & Aset">
      <div className="p-4 md:p-6 lg:p-8 overflow-y-auto w-full h-full custom-scrollbar bg-slate-50">
        
        {/* Header Actions */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tighter uppercase font-cyber italic">Permintaan Alat & Aset</h1>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">Pengajuan Kebutuhan Perlengkapan Kebun</p>
          </div>
          <Button variant="primary" className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700">
            <Plus size={16} /> Buat Pengajuan Aset
          </Button>
        </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 items-end">
            <div className="flex-1 w-full">
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Cari ID, Nama Alat..." className="w-full pl-9 pr-4 py-2 bg-slate-100 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all font-medium" />
              </div>
            </div>
        ...
        <div className="flex flex-col gap-4">
          {mockAssetRequests.map((req) => (
            <Card key={req.id} padding={false} className="overflow-hidden hover:border-blue-500/30 transition-colors">
              ...
              <div className="bg-slate-50 border-t border-slate-100 p-4 sm:px-6">
                <div className="relative">
                  ...
                  {req.status !== 'rejected' && (
                    <div 
                      className="absolute top-1/2 left-0 h-[2px] bg-blue-500 -translate-y-1/2 z-0 hidden sm:block transition-all duration-500" 
